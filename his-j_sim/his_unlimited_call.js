...
request('https://his-mobile.com/plan/sim-domestic/call-unlimited/', function (error, response, body) {
    if (error) {
        console.log('error');
    } else {
        ...
        const $planName = "call-unlimited";

        let data = body;
        let reg = /\('\.call-unlimited-price-([^-]+)-([a-z]{2})'\)\.html\('([0-9,]{3,5}円)'\)/g;
        let parts;
        let JsonFile = [];
...
function writeData(data) {
    ...
    fs.writeFile("call-unlimited-Plan.json", dataAsString, (err) => {
        if (err) {
            console.error(err);
            return;
        };

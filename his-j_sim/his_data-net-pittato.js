...
request('https://his-mobile.com/plan/sim-domestic/data-net-pitatto/', function (error, response, body) {
    if (error) {
        console.log('error');
    } else {
        ...
        const $planName = "data-net-pittato";

        let data = body;
        let reg = /\('\.data-net-pitatto-price-([^-]+)-([a-z]{2})'\)\.html\('([0-9,]{3,5}円)'\)/g;
        let parts;
        let JsonFile = [];
...
function writeData(data) {
    ...
    fs.writeFile("data-net-pittato-Plan.json", dataAsString, (err) => {
        if (err) {
            console.error(err);
            return;
        };
